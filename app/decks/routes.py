...
from .forms import AddCardForm, CreateDeckForm, EditCardForm
...
def decks():
    ...
    for deck in user_decks:
        ...
        learned_count = Card.query.filter(
            Card.deck_id == deck.deck_id, Card.box == 5, Card.next_review_date is None
        ).count()
...
def public_decks():
    ...
    for deck in shared_decks:
        card_count = Card.query.filter_by(deck_id=deck.deck_id).count()
        creator = User.query.get(deck.user_id).username
        deck_data.append(
            {
