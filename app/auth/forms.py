...
class RegistrationForm(FlaskForm):
    ...
    def validate_password(self, password):
        ...
        if re.search(r"(.)\1{2,}", password.data):
            raise ValidationError(
                "Password contains repetitive characters. Please choose a different one."
            )
        if re.search(r"(012|123|234|345|456|567|678|789)", password.data):
            raise ValidationError(
                "Password contains sequential characters. Please choose a different one."
            )
