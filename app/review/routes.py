...
def review_summary():
    ...
    if not session_id:
        flash(
            "No review session found. Please start a review session first.", "warning"
        )
    ...
    if not session.get(points_awarded_flag):
        ...
        points_earned = (total_cards_reviewed * base_card_points) + (
    ...
    if card.next_review_date is not None:
        ...
        for offset in range(1, 15):
            ...
            if card.box in REVIEW_SCHEDULE[next_cycle_day]:
                card.next_review_date = datetime.utcnow().date() + timedelta(
                    days=offset
                )
                break
        card.next_review_date = datetime.combine(
            card.next_review_date, datetime.min.time()
        )
    db.session.commit()
